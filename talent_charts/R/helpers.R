...
format_source <- function(src) {
  src %>%
    ...
    gsub("\\s+", " ", .) %>%
    trimws() %>%
    paste0("*Data from ", ., "* Chart by authors.")
}
...
save_chart <- function(plot,
  ...
  # Preserve the intended physical size in Word. With CHART_WIDTH_IN = 7.5 and
  # CHART_EXPORT_DPI = 300, the PNG is 2250 px wide but still inserts as 7.5 in.
