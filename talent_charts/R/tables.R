...
# US Letter width (8.5 in) - 0.5 in left margin - 0.5 in right margin = 7.5 in.
#
# Important distinction:
# - CSS px controls how large the table text appears after insertion in Word.
#   7.5 in * 96 CSS px/in = 720 CSS px, so 10 pt text renders as 10 pt
#   when the PNG is placed at 7.5 inches wide in Word.
# - Export is kept at 96 DPI / zoom 1 so pasted PNGs are natively 720 px wide.
#   This avoids Word/clipboard paste treating high-DPI PNGs as physically too wide
#   and clipping the right edge.
WORD_TABLE_WIDTH_IN <- 7.5
TABLE_CSS_DPI       <- 96
# Keep export DPI at 96 so pasted PNGs are natively 7.5 inches wide in Word.
# Higher-DPI + zoom often gets ignored by Word/clipboard paste and causes right-edge clipping.
TABLE_EXPORT_DPI    <- 96
TABLE_CSS_WIDTH_PX  <- round(WORD_TABLE_WIDTH_IN * TABLE_CSS_DPI)
TABLE_EXPORT_PX     <- TABLE_CSS_WIDTH_PX
TABLE_EXPORT_ZOOM   <- 1

# Previous versions used a 1500px CSS table width. These helpers scale old
# hand-tuned column widths down to the new 720px CSS coordinate system.
TABLE_LEGACY_CSS_WIDTH_PX <- round(WORD_TABLE_WIDTH_IN * 200)
# Keep hand-tuned column widths in the same CSS coordinate system as the table.
# Earlier versions scaled these down from a 1500px table, which made stub/label
# columns too narrow and caused row labels to break into unreadable fragments.
TABLE_WIDTH_SCALE         <- 1
...
# Font sizes are specified in document points, then converted to CSS px because
# gt/webshot use browser CSS sizing.
table_pt_to_px <- function(pt) pt * TABLE_CSS_DPI / 72

TABLE_DATA_FONT_PT     <- 10
TABLE_HEADER_FONT_PT   <- 10
TABLE_TITLE_FONT_PT    <- 12
TABLE_SUBTITLE_FONT_PT <- 11
TABLE_SOURCE_FONT_PT   <- 11
...
# ---- Width profiles --------------------------------------------------------
# Standard tables render at 720 CSS px = 7.5 inches at 96 DPI.
# Wide/ultrawide tables render on a larger CSS canvas, then write matching PNG
# density metadata so Word still places them at 7.5 inches. Font px is scaled
# with the profile DPI so apparent document font sizes remain stable.
TABLE_WIDTH_PROFILES <- list(
  standard = list(width_px = TABLE_CSS_WIDTH_PX, stub_px = 210, dpi = 96,  data_pt = 10, header_pt = 10, title_pt = 12, subtitle_pt = 11, source_pt = 11, pad_px = 5, col_pad_px = 5),
  wide     = list(width_px = 900,                stub_px = 260, dpi = 120, data_pt = 10, header_pt = 10, title_pt = 12, subtitle_pt = 11, source_pt = 11, pad_px = 5, col_pad_px = 4),
  ultrawide= list(width_px = 1080,               stub_px = 285, dpi = 144, data_pt = 10, header_pt = 10, title_pt = 12, subtitle_pt = 11, source_pt = 11, pad_px = 4, col_pad_px = 3)
)
...
infer_table_profile <- function(name, gt_tbl = NULL) {
  explicit <- attr(gt_tbl, "tpa_profile")
  if (!is.null(explicit)) return(explicit)
  # Known tables that have more columns than fit comfortably in a 720px canvas.
  if (name %in% c("chapter2_tab_008", "chapter5_tab_020")) return("ultrawide")
  if (name %in% c("chapter1_tab_006d", "chapter2_tab_010")) return("wide")
  "standard"
}
...
apply_table_profile <- function(gt_tbl, profile = "standard", stub_width_px = NULL) {
  p <- table_profile(profile)
  # Convert document-point sizes to the CSS px needed for this profile DPI.
  pt_to_px_profile <- function(pt) pt * p$dpi / 72
  
  gt_tbl %>%
    tab_options(
      table.font.size        = px(pt_to_px_profile(p$data_pt)),
      table.width            = px(p$width_px),
      container.width        = px(p$width_px),
      column_labels.font.size = px(pt_to_px_profile(p$header_pt)),
      column_labels.padding   = px(p$col_pad_px),
      data_row.padding        = px(p$pad_px),
      heading.title.font.size    = px(pt_to_px_profile(p$title_pt)),
      heading.subtitle.font.size = px(pt_to_px_profile(p$subtitle_pt)),
      source_notes.font.size     = px(pt_to_px_profile(p$source_pt))
    ) %>%
    fixed_table_width(profile = profile, stub_width_px = stub_width_px %||% attr(gt_tbl, "tpa_stub_width_px") %||% p$stub_px) %>%
    scale_columns_to_width(profile = profile, stub_width_px = stub_width_px) %>%
...
# Circular flag PNGs live in flags/<slug>.png, paths relative to the knit
...
flag_path_for <- function(country) {
  ...
  ifelse(is.na(slug), NA_character_, paste0("flags/", slug, ".png"))
}
...
build_region_table <- function(name, meta,
  ...
  tbl_df <- tibble(
    ...
    mutate(
      PctChangeCount = table_change(FirstYearCount, LastYearCount, change_type),
      
      PctChangeShare = table_change(FirstYearShare, LastYearShare, change_type)
    ) %>%
  ...
  tbl_df %>%
    ...
    fmt_number(
      ...
      decimals = if (change_type == "relative") 1 else 0,
      force_sign = TRUE,
    ...
    tab_footnote(
      footnote = paste(
        ...
        if (change_type == "relative")
          "Change is relative change between"
        else
          "Change is absolute count change for enrollment and percentage-point change for share between",
...
build_sector_bookend_table <- function(meta,
                                       sources = c(`U.S. citizens`          = "AF61",
                                                   `Temporary visa holders` = "AF60"),
  ...
  tbl %>%
    ...
    tab_footnote(
      footnote = paste(
        ...
        "enrolled in the given field in 2009 and 2024, and the",
...
# Companion to fig_125. Same shape as build_occupation_share_table (two
# bookend levels, interior year-over-year deltas, and a net change), but
# levels are dollar amounts and rows are grouped by country (U.S. top,
# China bottom). df arrives pivoted wide with a Country group column, a Type
# stub, dollar levels in level_cols, change columns in chg_cols, and a net
# change column named Change. Ordering and grouping happen upstream.
build_rd_type_delta_table <- function(df, name, meta, level_cols, chg_cols,
                                      spanner_label = "Gross domestic R&D expenditure (USD billions, PPP)",
                                      change_type = c("relative", "ppt"),
                                      footnote = NULL) {
  ...
  if (is.null(footnote)) {
    footnote <- paste(
      "The first and last columns are spending levels in billions of US dollars",
      "(PPP converted). The interior columns show the",
      ...
      "change type across the full window. Basic research is investment in future capability;",
      "applied research and experimental development are nearer-term."
    )
  }
  ...
  tbl <- df %>%
    ...
    fmt_number(columns = all_of(all_chg_cols), decimals = 1,
               force_sign = TRUE,
               pattern = if (change_type == "relative") "{x}%" else "${x}B") %>%
    tab_spanner(label = spanner_label, columns = all_of(span_order)) %>%
    cols_label(!!!level_labels, !!!chg_labels, Change = "Net Change")
  
  # per-column symmetric diverging gradient, as in build_occupation_share_table
  for (col in all_chg_cols) {
    lim <- max(abs(df[[col]]), na.rm = TRUE)
    tbl <- tbl %>%
      data_color(columns = all_of(col),
  ...
  tbl %>%
    ...
    tab_style(style = cell_text(weight = "bold"),
              locations = cells_body(columns = Change)) %>%
    cols_width(Type ~ px(table_scaled_px(230))) %>%
    tab_footnote(footnote = footnote,
                 locations = cells_column_spanners(spanners = spanner_label)) %>%
    theme_gt_tpa(meta = meta) %>%
    opt_css(css = sprintf(
...
# ---- Patents-by-field summary table -------------------------
# Combines the three patent line charts (fig_059 biotech, fig_060
# semiconductors, fig_061 computer tech) into one table. Reads each wide
# file (Year + one column per country office), pulls two bookend years,
# and reports grants in each year plus the change, grouped by field.
# All seven offices report across all three fields; "—" only appears if a
# given year is missing. Rows ordered by total last-year grants.
#   name     : gt id + opt_css selector
#   sources  : named vector c(<Field label> = <fig key>), in column order
# ---- Patents-by-field summary table -------------------------
build_patents_table <- function(name, meta, sources,
  ...
  tbl <- tbl %>%
    cols_label(
      ends_with("_First")  ~ as.character(first_year),
      ends_with("_Last")   ~ as.character(last_year),
      ends_with("_Change") ~ "Change"
    )
  ...
  tbl %>%
    ...
    tab_style(style = cell_text(align = "left"),
              locations = cells_stub(rows = TRUE)) %>%
    cols_width(
      Country              ~ px(table_scaled_px(140)),
      ends_with("_First")  ~ px(table_scaled_px(66)),
      ends_with("_Last")   ~ px(table_scaled_px(66)),
    ...
    tab_footnote(
      footnote = paste0(
        ...
        "Countries are ordered by total ", last_year,
...
# then name). Three grant counts shown as levels with the change between
# each successive pair interleaved, plus a net change. Levels are integer
# counts; change columns get a per-column symmetric diverging scale.
# df arrives pivoted wide with columns in display order:
# Company, flag, <level>, <chg>, <level>, <chg>, <level>, Change, where
# flag holds relative paths to flags/<slug>.png. Merging the flag into the
# stub keeps it off the spanner row, so no red rule appears above it.
...
save_table <- function(gt_tbl, name, size = NULL, profile = NULL, stub_width_px = NULL) {
  ...
  html_dir <- file.path("final_tables", "html")
  png_dir  <- file.path("final_tables", "pngs")
  ...
  gt_tbl <- apply_table_profile(gt_tbl, profile = profile, stub_width_px = stub_width_px)
  
  height_scale <- p$width_px / TABLE_CSS_WIDTH_PX
  vheight <- ceiling(unname(TABLE_EXPORT_HEIGHTS_PX[[size]]) * height_scale)
  ...
  # Force Word to place every output at 7.5 inches wide. For wide profiles the
  # pixel canvas is larger, but density is larger too, so the physical width and
  # apparent font sizes remain constant.
  if (requireNamespace("magick", quietly = TRUE)) {
    img <- magick::image_read(png_path)
    word_dpi <- round(p$width_px / WORD_TABLE_WIDTH_IN)
    magick::image_write(
      img,
      path    = png_path,
      density = paste0(word_dpi, "x", word_dpi)
    )
  }
